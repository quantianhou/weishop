...
function cloud_prepare() {
	...
	if(empty($_W['setting']['site']['key']) || empty($_W['setting']['site']['token'])) {
		return error('-1', "您的站点只有在云服务平台成功注册后，才能使用云服务的相应功能。");
	}
...
function cloud_m_prepare($name) {
	...
	$pars['module'] = $name;
	$dat = cloud_request('http://addons.mitusky.com/api/api.php', $pars);
	if (is_error($dat)) {
		return $dat;
	}
...
function cloud_sms_send($mobile, $content, $postdata = array()) {
	...
	$response = cloud_request('http://addons.mitusky.com/api/api.php', $pars);
...
function cloud_download($path, $type = '') {
	...
	$dat = cloud_request('http://addons.mitusky.com/api/api.php', $pars, $headers, 300);
	if(is_error($dat)) {
...
function _cloud_shipping_parse($dat, $file) {
	...
	if ($dat['content'] == 'blacklist') {
		return error(-1, '抱歉，您的站点已被列入云服务黑名单，云服务一切业务已被禁止！');
	}
...
function cloud_cron_create($cron) {
	...
	$result = cloud_request('http://addons.mitusky.com/api/api.php', $pars);
...
function cloud_auth_url($forward, $data = array()){
	...
	$auth_url = 'http://addons.mitusky.com/web/auth.php?__auth=' . $query;
...
function cloud_flow_master_post($flow_master) {
	...
	$dat = cloud_request('http://addons.mitusky.com/api/api.php', $pars, array(), 300);
	if(is_error($dat)) {
		return error(-1, '网络存在错误， 请稍后重试。' . $dat['message']);
	}
