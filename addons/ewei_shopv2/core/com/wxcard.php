...
class Wxcard_EweiShopV2ComModel extends ComModel
{
    ...
    public function createCard($params)
    {
        ...
        else if ($card_type == 'MEMBER_CARD')
        {
            ...
            $wx_activate = $params['wx_activate'];
        ...
        else if ($card_type == 'MEMBER_CARD')
        {
            ...
            $jsonData .= ',"wx_activate":' . $wx_activate;
            if (!(empty($custom_field1)))
            {
    ...
    public function membercardmanager($carddata, $card_id = '')
    {
        ...
        //fanhailong add 当当前公众号是【君梓店员之家】（就是店员d端公众号），会员卡的按钮和跳转链接修改为d端的个人中心
        if($_W['uniaccount']['original'] == 'gh_8ec8dbfd8d99'){
        ...
        if (empty($card_id))
        {
            $create_result = $this->createCard($params);
            if($create_result['errcode'] != 0){//如果有报错，返回微信的结果，让前端控制器去报错
                return $create_result;
            }
            if($_W['uniaccount']['original'] == 'gh_8ec8dbfd8d99'){
                //fanhailong add，当当前公众号是【君梓店员之家】（就是店员d端公众号）时：创建会员卡后，直接设置会员卡激活属性,用于d端店员绑定手机号时直接调取微信会员卡信息
                load()->classs('coupon');
                $coupon_api = new coupon($_W['uniaccount']['acid']);
