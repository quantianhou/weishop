...
class Index_EweiShopV2Page extends WebPage
{
	public function main($goodsfrom = '')
	{
		...
		if (!empty($total)) {
			$sql = 'SELECT g.* FROM ' . tablename('ewei_shop_goods') . 'g' . $sqlcondition . $condition . $groupcondition . " ORDER BY g.`status` ASC, g.`displayorder` ASC,\r\n                g.`id` DESC LIMIT " . (($pindex - 1) * $psize) . ',' . $psize;
			$list = pdo_fetchall($sql, $params);

			foreach ($list as $key => &$value) {
				$url = mobileUrl('goods/detail', array('id' => $value['id']), true);
				$value['qrcode'] = m('qrcode')->createQrcode($url);
                $value['shopname'] = pdo_fetchcolumn('select store_short_name as storename from ' . tablename('ewei_shop_store') . ' where id=:storeid limit 1', array(':storeid' => $value['shop_id']));
            }
