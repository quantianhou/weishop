...
class Detail_EweiShopV2Page extends MobilePage 
{
	public function main() 
	{
        ...
        $cm_storeids = implode(',',explode(',',trim($coupon['storeid'], ',')));
        if(isset($coupon['coupontype']) && $coupon['coupontype'] == 2)
        {
            $shoplist = pdo_fetchall('select *  from ' . tablename('ewei_shop_store') . ' og  where og.uniacid=:uniacid and og.id IN('.$cm_storeids.') and type in(2,3)', array(':uniacid' => $_W['uniacid']));
        }else{
            $shoplist = pdo_fetchall('select *  from ' . tablename('ewei_shop_store') . ' og  where og.uniacid=:uniacid and og.id IN('.$cm_storeids.')', array(':uniacid' => $_W['uniacid']));
        }
        $shop_count = count($shoplist);
