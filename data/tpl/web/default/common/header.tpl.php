...
<div class="container">
	<?php  if(in_array(FRAME, array('account', 'system', 'adviertisement', 'wxapp')) && !in_array($_GPC['a'], array('news-show', 'notice-show'))) { ?>
	...
	<div class="panel panel-content main-panel-content <?php  if(!empty($frames['section']['platform_module_menu']['plugin_menu'])) { ?>panel-content-plugin<?php  } ?>">
		<div class="content-head panel-heading main-panel-heading">
			<?php  if(($_GPC['c'] != 'cloud' && !empty($_GPC['m']) && !in_array($_GPC['m'], array('keyword', 'special', 'welcome', 'default', 'userapi', 'service'))) || defined('IN_MODULE')) { ?>
				...
				<img src="<?php  echo tomedia("addons/".$_W['current_module']['name']."/icon-custom.jpg")?>" class="head-app-logo">
				<?php  } else { ?>
				<img src="<?php  echo tomedia("addons/".$_W['current_module']['name']."/icon.jpg")?>" class="head-app-logo">
				<?php  } ?>
			...
			<?php  if(FRAME == 'wxapp') { ?>
				...
				<div class="pull-right">
					<a href="<?php  echo url('wxapp/version/display', array('uniacid' => $wxapp_info['uniacid'], 'acid' => $wxapp_info['acid']))?>" class="color-default"><i class="wi wi-cut"></i>切换版本</a>
					<a href="<?php  echo url('wxapp/manage/display', array('uniacid' => $wxapp_info['uniacid'], 'acid' => $wxapp_info['acid']))?>" class="color-default"><i class="wi wi-text"></i>管理</a>
		...
		<div class="left-menu">
			...
			<?php  if(empty($frames['section']['platform_module_menu']['plugin_menu'])) { ?>
				...
				<div class="panel panel-menu">
					...
					<ul class="list-group collapse <?php  if(!empty($_W['setting']['copyright']['skin']) && $_W['setting']['copyright']['skin'] == 'black') { ?><?php  if($_GPC['menu_fold_tag:'.$frame_section_id] == 0) { ?>in<?php  } ?><?php  } else { ?>in<?php  } ?>" id="frame-<?php  echo $frame_section_id;?>">
