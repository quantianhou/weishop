<?php defined('IN_IA') or exit('Access Denied');?><div class="panel we7-panel">
	...
	<input type="hidden" name="reply[reply_module]" value="">
		...
		<div class="we7-select-msg">
			<div class="add" onclick="$(this).addClass('hide').next('ul').removeClass('hide');"><i class="wi wi-registersite"></i>添加回复</div>
...
<script>
	...
	function select_wxcard(){
        ...
        $('.keywords-list').append(
            ...
            '					<span class="title">[微信会员卡]</span>'+
            '				</a>'+
            '				<p class="desc"><a href="javascript:;" class="appmsg-desc">'+keywords+'</a></p>'+
            '		</div>'+
            '	</div>'+
            '	<div class="opr">'+
            '		<a href="javascript:;" data-media="微信会员卡" class="del-gray" onclick="delmedia(\''+type+'\', this)">删除</a>'+
            '	</div>'+
            '</div>'+
            '</li>'
        );
			...
			if (type == 'basic') {
				if (angular.isDefined(val)) {
					...
					for (var i = 0; i < editmedia.length; i++) {
						if (htmlEncode($(editmedia.get(i)).find('.edit-gray').data('media')) == htmlEncode(angular.toJson(val))) {
							...
							var index = _.indexOf(inputValArr, htmlEncode(angular.toJson(val)));
