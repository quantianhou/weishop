...
<div class="page-content">
	<form action="./index.php" method="get" class="form-horizontal table-search" role="form" id="form1">
		...
		<div class="page-toolbar">
			...
			<span class='col-sm-4'>
				<?php  echo tpl_coupon_type_selector('coupon_type',
						[
							'name' => '优惠券类型',
							'div_name' => 'coupon_type',
							'buttontext'=>'添加优惠券',
							'item' =>[
								['type'=>0,'title'=>'购物优惠券','desc'=>'支持满减券/折扣券'],
								['type'=>1,'title'=>'充值优惠券','desc'=>'充值时可抵扣部分金额'],
								['type'=>2,'title'=>'门店兑换券','desc'=>'仅支持到店扫码验证使用'],
							]
						]
                        )?>
			</span>
	...
	<table class="table table-hover table-responsive">
		...
		<tr>
			...
			<td><?php  if($row['coupontype']==0) { ?>
				...
				<label class='label label-info'>兑换券</label>
				<?php  } ?>
...
<script>
	function sendCoupon() {
        var type = $("input[name='radio_coupon_type']:checked").val();
        switch (type)
		{
			case '0':
			    window.location.href = "<?php  echo webUrl('sale/coupon/add')?>";
			    break;
			case '1':
                window.location.href = "<?php  echo webUrl('sale/coupon/add',array('type'=>1))?>";
                break;
			case '2':
                window.location.href = "<?php  echo webUrl('sale/coupon/add',array('type'=>2))?>"
                break;
			default:
			    alert('请选择优惠券类型');
		}
